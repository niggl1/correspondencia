// app/aprovar-moradores/page.tsx
'use client'
...
export default function AprovarMoradores() {
  ...
  const carregarMoradores = async (uid: string) => {
    try {
      // Buscar moradores pendentes de aprovação
      const q = query(
        collection(db, 'users'),
        where('role', '==', 'morador'),
  ...
  // ✅ NOVO: Função para enviar email de aprovação
  ...
  const aprovarMorador = async (moradorId: string) => {
    ...
    try {
      ...
      // ✅ NOVO: Enviar email de aprovação
      if (moradorData) {
              ...
              >
                ...
                <div className="mt-6 flex gap-4">
                  <button
                    ...
                    className="flex-1 bg-primary-600 text-white px-6 py-3 rounded-lg hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed font-semibold transition-colors"
                  >
                    {processando === morador.id ? 'Processando...' : '✅ Aprovar'}
                  </button>
