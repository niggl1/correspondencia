...
import { Save, RefreshCw, Info, AlertTriangle, Lock, Unlock } from 'lucide-react';
...
export default function TemplateManager({ condoId }: TemplateManagerProps) {
  ...
  const insertVariable = (variable: string) => {
    setLocalTemplate(prev => ({ ...prev, content: (prev.content || '') + ` {${variable}}` }));
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-green-100">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={refresh}><RefreshCw className="w-4 h-4" /></button>
      </div>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`px-4 py-2 rounded-lg font-bold whitespace-nowrap ${activeCategory === cat.key ? 'bg-[#057321] text-white' : 'bg-gray-100 text-gray-600'}`}
          >
            {cat.label}
          </button>
        ))}
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2 p-2 bg-gray-50 border rounded-lg">
          {getAvailableVariables(activeCategory).map(v => (
            <button key={v} onClick={() => insertVariable(v)} className="text-xs font-bold bg-white border px-2 py-1 rounded text-[#057321]">
              {`{${v}}`}
            </button>
          ))}
        </div>
        ...
        <div className="border-4 border-red-500 p-4 rounded-xl bg-red-50">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
                <div className="p-2 bg-white rounded-full border">
                    {localTemplate.isMandatory ? <Lock size={24} className="text-red-600"/> : <Unlock size={24} className="text-green-600"/>}
                </div>
                <div>
