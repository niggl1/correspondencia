...
export default function GerenciarCondominios() {
  ...
  const excluirCondominio = async (c: Condominio) => {
    if (!confirm(`ATENÇÃO: Excluir "${c.nome}" apagará TODOS os dados vinculados (blocos, moradores, etc). Continuar?`)) return;
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={4} className="p-8 text-center text-gray-500">Nenhum condomínio encontrado.</td></tr>
              ) : (
                  ...
                  <tr key={c.id} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {c.logoUrl ? (
                          <img src={c.logoUrl} alt={c.nome} className="w-10 h-10 rounded-lg object-cover border" />
                        ) : (
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase ${c.status === 'ativo' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {c.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                      <button onClick={() => editarCondominio(c)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="Editar">
                        <Edit2 size={18} />
                      </button>
                      <button onClick={() => alternarStatus(c)} className={`p-2 rounded-lg ${c.status === 'ativo' ? 'text-orange-600 hover:bg-orange-50' : 'text-green-600 hover:bg-green-50'}`} title={c.status === 'ativo' ? 'Desativar' : 'Ativar'}>
                        <Power size={18} />
                      </button>
                      <button onClick={() => excluirCondominio(c)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg" title="Excluir">
                        <Trash2 size={18} />
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
            <div className="bg-gray-50 p-4 border-b flex justify-between items-center">
              <h3 className="font-bold text-lg text-gray-800">{condominioEditando ? "Editar Condomínio" : "Novo Condomínio"}</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full pl-10 p-2.5 border rounded-lg focus:ring-2 focus:ring-[#057321]" placeholder="Ex: Edifício Solar" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <textarea value={endereco} onChange={(e) => setEndereco(e.target.value)} className="w-full pl-10 p-2.5 border rounded-lg focus:ring-2 focus:ring-[#057321]" placeholder="Rua, Número, Bairro..." rows={3} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="url" value={logoUrl} onChange={(e) => setLogoUrl(e.target.value)} className="w-full pl-10 p-2.5 border rounded-lg focus:ring-2 focus:ring-[#057321]" placeholder="https://..." />
                </div>
                {logoUrl && (
                  <div className="mt-2 flex justify-center">
                    <img src={logoUrl} alt="Preview" className="h-16 w-16 rounded-lg object-cover border" onError={(e) => (e.currentTarget.style.display = 'none')} />
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-4 bg-gray-50 border-t flex justify-end gap-3">
              <button onClick={() => setModalAberto(false)} className="px-4 py-2 text-gray-700 hover:bg-gray-200 rounded-lg font-medium">Cancelar</button>
              <button onClick={salvarCondominio} disabled={loading} className="px-6 py-2 bg-[#057321] text-white rounded-lg font-bold hover:bg-[#046019] disabled:opacity-50">
